...
with open('./dev_data/dev.csv', 'r') as f:
    ...
    for row in reader:
        ...
        targetedLabel.append(row['targetedLabel'])
...
for adv_f in adv_files:
    ...
    if label[0] == int(targetedLabel[i]):
        with open(dir2 + adv_f, 'rb') as f:
            raw_image = imread(f, mode='RGB').astype(np.float)
            image = imresize(raw_image, [224, 224]) / 255.0
            img = image * 2.0 - 1.0
        score.append(np.linalg.norm(adv-img))
        label2 = sess.run(pred, feed_dict={x: img.reshape(1, 224, 224, 3)})
        if label2[0] != int(trueLabel[i]):
            print(label2[0])
            print(trueLabel[i])
            print('error')
        success += 1
